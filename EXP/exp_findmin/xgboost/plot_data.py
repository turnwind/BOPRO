...
data_RS = json.load(open('EXP/exp_findmin/xgboost/RS.json'))
data_BO = json.load(open('EXP/exp_findmin/xgboost/BO.json'))
data_LLM = json.load(open('EXP/exp_findmin/xgboost/LLM.json'))
data_BOPRO = json.load(open('EXP/exp_findmin/xgboost/BOPRO.json'))
...
plt.fill_between(x_values, np.array(average_RS) - np.array(std_RS), np.array(average_RS) + np.array(std_RS), alpha=0.3)
x_values = np.arange(len(data_BO[0]))
plt.plot(x_values, average_BO, label='BO')
plt.fill_between(x_values, np.array(average_BO) - np.array(std_BO), np.array(average_BO) + np.array(std_BO), alpha=0.3)
