...
def warm_strat(num):
    ...
    while flag:
        pt = (
            ...
            "x1 must be within the range of [-5, 5]. x2 must be within the range of [-5, 5]. x3 must be within the range of [-5, 5]."
...
if __name__ == "__main__":
    ...
    arrloss = [1e6]
    for i in inidatas:
        loss = obj(i[0],i[1],i[2])
        new_entry = {"params": [i[0], i[1],i[2]], "loss": loss}
    ...
    for i in tqdm(range(numiters)):
        ...
        loss = obj(next_point["params"][0],next_point["params"][1],next_point["params"][2])
        arrloss.append(loss)
...
data = {}
with open("EXP/exp_findmin/cubic_function/BOPRO.json","r") as f:
    data = json.load(f)

data["loss"].append(arrloss)
with open("EXP/exp_findmin/cubic_function/BOPRO.json","w") as f:
    json.dump(data,f)
