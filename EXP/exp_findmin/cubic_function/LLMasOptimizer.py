...
if __name__ == "__main__":
    ...
    for i in range(13):
        ...
        datas.append({"params": point, "loss": obj(point[0],point[1],point[2])})
    ...
    for i in range(len(data)):
        data[i] = min(data[:i+1])
    # 打开json文件
    with open("EXP/exp_findmin/cubic_function/LLM.json", "r") as f:
        data1 = json.load(f)
        data1["loss"].append(data)
    # 写入json文件
    with open("EXP/exp_findmin/cubic_function/LLM.json", "w") as f:
        json.dump(data1, f)
